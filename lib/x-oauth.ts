...
export async function exchangeAuthorizationCode(code:string,verifier:string,clientId:string,redirectUri:string):Promise<TokenResponse | null> {
  ...
  const response = await getXTransport().request<TokenResponse>({path:"/2/oauth2/token",method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded",...tokenAuth(clientId,body)},body,accounting:{kind:"request",endpoint:"oauth.token"}});
...
export async function refreshXAccessToken(session:XSession):Promise<XSession | null> {
  ...
  const response = await getXTransport().request<TokenResponse>({path:"/2/oauth2/token",method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded",...tokenAuth(session.clientId,body)},body,accounting:{kind:"request",endpoint:"oauth.refresh"}});
