...
export async function GET(request:NextRequest){const denied=await authorizeBrowserOrApiRead(request);if(denied)return denied;if(deploymentPosture()==="demo")return NextResponse.json({feedback:[]});return NextResponse.json({feedback:await getDb().select().from(feedback).orderBy(desc(feedback.createdAt)).limit(500)})}
