...
export async function GET(request:NextRequest) {
  ...
  try {
    ...
    let me = await xFetch<{data:XUser}>("/2/users/me?user.fields=name,username,profile_image_url,public_metrics",session.accessToken);
    let refreshed = false;
    if (me.status === 401) {
      const next = await refreshXAccessToken(session);
      if (!next) return NextResponse.json({error:"X_RECONNECT_REQUIRED"},{status:401});
      session = next; refreshed = true; await storeXSession(session); me = await xFetch<{data:XUser}>("/2/users/me?user.fields=name,username,profile_image_url,public_metrics",session.accessToken);
    }
    ...
    const payload:SyncPayload = {source:"live",syncedAt:new Date().toISOString(),account:{id:me.data.data.id,name:me.data.data.name,username:me.data.data.username,profileImageUrl:me.data.data.profile_image_url,followersCount:me.data.data.public_metrics?.followers_count},opportunities,ideas:generateIdeas(feed,ownPosts),usage:await getUsage()};
